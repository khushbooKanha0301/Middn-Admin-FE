...
function Sidebar() {
  ...
  const SidebarMenu = [
    ...
    },
    {
      ...
      title:"User List",
      link:'/userlist',
      activeRoutes:['userdetails','userdeposithistory','transactionlogs','userloginhistory']
    },
    {
      icon:Activity,
      title:"Manage Escrow",
  ...
  return (
    <div className='mainSidebar bg-[#131517] border-r-[#454545] border-r border-solid'>
      ...
      <div className='mt-[-5px]'>
        <span className='text-[12px] text-[#808191] mb-[16px] inline-block iconApp'>Ico App</span>
        <ul className='menu'>
          {
            SidebarMenu.map((menuItem, index) => (
              <li key={index}>
                <NavLink className={menuItem.activeRoutes.includes(routeName) ? 'active nav-link' : 'nav-link'} to={menuItem.link}>
                  <span className='icon'>
